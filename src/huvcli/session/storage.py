"""Workspace files: prefs.json, conversations.jsonl, last_conversation.json."""
...
def save_conversation(cwd: Path, messages: list[dict[str, Any]], max_messages: int = 200) -> None:
    """Persist conversation for /resume.

    Drops the system message (agent re-prepends fresh on resume) and caps
    to the last `max_messages` non-system messages so the file doesn't
    grow without bound. Snaps the cut forward past `tool` messages so we
    don't orphan a tool_call_id reference.
    """
    payload = [m for m in messages if m.get("role") != "system"]
    if len(payload) > max_messages:
        cut = len(payload) - max_messages
        while cut < len(payload) and payload[cut].get("role") == "tool":
            cut += 1
        payload = payload[cut:]
